...
namespace ProntuarioApp.Views
{
    ...
    public partial class Cirurgias : ContentPage, INotifyPropertyChanged
    {
        private readonly PacienteApiService _pacienteApiService;

        public Cirurgias(Paciente paciente)
        {
            InitializeComponent();
            ...
            ListarCirurgias(paciente.Id);
        }


        private async void ListarCirurgias(int id)
        {
            try
            {
                aparece();

                var result = await _pacienteApiService.ListarCirurgias(id);
                if (result.Success)
                {
                    some();
                    CirurgiaResponse.ItemsSource = result.Result.ToList();
